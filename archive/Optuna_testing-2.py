...
data = formatrix.DataCurator("./bdf9_w_rollingavg.csv")
...
class Net(nn.Module):
    ...
    def __init__(self, trial, num_conv_layers, num_filters, num_neurons1, num_neurons2, 
        ...
        out_size_width = in_size_width - (kernel_size - 1) - 1 \
                         + 2*padding_size                                               # Width of the output kernel
        out_size_width = math.floor((out_size_width / 2) + 0.5)                                        # Width after pooling
        
        out_size_height = in_size_height - (kernel_size - 1) - 1 \
                          + 2*padding_size                                              # Height of the output kernel
        out_size_height = math.floor((out_size_height / 2) + 0.5)                                      # Height after pooling
...
#         self.fc3.bias.data(np.log(41/909)
...
def objective(trial):
    ...
    num_filters = [int(trial.suggest_discrete_uniform("num_filter_"+str(i), 2, 80, 1))
...
if __name__ == '__main__':
    ...
    df.to_csv('./optuna_res/optuna_results_all-rolling-avgs'+str(0)+'.csv', index=False)  # Save to csv file
